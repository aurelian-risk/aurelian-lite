...
export function makeSampleStudy(): Study {
  ...
  add("requirement", { name: "Risk analysis and information system security policies", ref_id: "21(2)(a)", framework: "NIS2", scope: "in scope", category: "Governance" }); // intentionally left uncovered (gap demo)
  const reqIncident = add("requirement", { name: "Incident handling", ref_id: "21(2)(b)", framework: "NIS2", scope: "in scope", category: "Operations" });
  const reqBackup = add("requirement", { name: "Business continuity (backup, disaster recovery, crisis management)", ref_id: "21(2)(c)", framework: "NIS2", scope: "in scope", category: "Resilience" });
  const reqAuth = add("requirement", { name: "Multi-factor / continuous authentication and secured communications", ref_id: "21(2)(j)", framework: "NIS2", scope: "in scope", category: "Access" });
  const reqData = add("requirement", { name: "Data Security", ref_id: "PR.DS", framework: "NIST CSF", scope: "in scope", category: "Protect" });
  const reqAC = add("requirement", { name: "Access Control", ref_id: "AC", framework: "NIST 800-53", scope: "in scope", category: "Control family" });
  add("requirement", { name: "Contingency Planning", ref_id: "CP", framework: "NIST 800-53", scope: "in scope", category: "Control family" }); // gap demo (radar shows partial coverage)
  // Enough of the scope to be a realistic compliance table rather than a token one: three
  // frameworks, several categories, and more rows than fit at a glance. Eight are answered
  // for here; the rest are in the catalogue but out of this study's scope, which is what a
  // framework normally looks like against one system.
  const reqSupply = add("requirement", { name: "Supply chain security", ref_id: "21(2)(d)", framework: "NIS2", scope: "in scope", category: "Supply chain" });
  add("requirement", { name: "Basic cyber hygiene and cybersecurity training", ref_id: "21(2)(g)", framework: "NIS2", scope: "out of scope", category: "People" });
  add("requirement", { name: "Cryptography and, where appropriate, encryption", ref_id: "21(2)(h)", framework: "NIS2", scope: "out of scope", category: "Protection" });
  add("requirement", { name: "Human resources security, access control and asset management", ref_id: "21(2)(i)", framework: "NIS2", scope: "out of scope", category: "Access & assets" });
  add("requirement", { name: "Identity Management, Authentication, and Access Control", ref_id: "PR.AA", framework: "NIST CSF", scope: "out of scope", category: "Protect" });
  add("requirement", { name: "Continuous Monitoring", ref_id: "DE.CM", framework: "NIST CSF", scope: "out of scope", category: "Detect" });
  add("requirement", { name: "Audit and Accountability", ref_id: "AU", framework: "NIST 800-53", scope: "out of scope", category: "Control family" });

  // ── Workshop 5 - Treatment ──
  add("security_measure", { name: "Secure email gateway & phishing training", description: "A secure email gateway filters malicious attachments and links, backed by regular phishing-awareness training so staff recognise and report the lures that would otherwise deliver the initial loader through the maintenance channel.", scope: "in use", measure_type: "Preventive", status: "Implemented", priority: 2, implementation_level: 4, strength: 3, mitigations: "M1021, M1049, M1054, M1017", covers: [st1], protects: [saHis] });
  add("security_measure", { name: "MFA on remote maintenance access", description: "Phishing-resistant multi-factor authentication enforced on all remote and third-party maintenance access, so stolen or phished credentials alone cannot open a session or be replayed after a credential dump.", scope: "in use", measure_type: "Preventive", status: "Planned", priority: 3, implementation_level: 2, strength: 4, mitigations: "M1032", covers: [stLateral], protects: [saHis, saNetwork], fulfills: [reqAuth, reqAC, reqSupply] });
  add("security_measure", { name: "Network segmentation (IT / clinical VLANs)", description: "The clinical VLANs are firewalled off from the corporate IT network so that an attacker who lands in IT cannot move laterally into the ward and medical-device networks unimpeded, containing the blast radius of an intrusion.", scope: "in use", measure_type: "Preventive", status: "Implemented", priority: 3, implementation_level: 3, strength: 3, mitigations: "M1030", covers: [stLateral], protects: [saNetwork] });
  add("security_measure", { name: "Egress monitoring & DLP", description: "Egress monitoring and data-loss-prevention rules detect and block bulk transfers of health records to external destinations, whether staged over a web service before ransomware or copied to removable media by an insider.", scope: "in use", measure_type: "Detective", status: "Planned", priority: 3, implementation_level: 2, strength: 2, mitigations: "M1057", covers: [stExfil, stI3], protects: [saHis] });
  add("security_measure", { name: "EDR on clinical endpoints", description: "Endpoint detection and response on clinical endpoints and servers flags the tell-tale behaviour of credential dumping, suspicious scheduled tasks and mass file encryption, giving the SOC a chance to contain the intrusion before impact.", scope: "in use", measure_type: "Detective", status: "Implemented", priority: 3, implementation_level: 3, strength: 3, mitigations: "M1040, M1049", covers: [st2, st3, st6, stI2], protects: [saHis], fulfills: [reqIncident] });
  add("security_measure", { name: "Offline immutable backups", description: "Air-gapped, immutable backups of the patient database with regular restore drills, so that even if the Hospital Information System is encrypted the hospital can recover within hours rather than paying a ransom.", scope: "in use", measure_type: "Corrective", status: "Implemented", priority: 4, implementation_level: 4, strength: 4, mitigations: "M1053, M1029", covers: [st6], protects: [saBackup], fulfills: [reqBackup, reqData] });
  ...
  return {
    ...
    sector: "Healthcare",
    // A regional hospital: medium by headcount class is the unit the rates describe, and
    // an exercised incident-response plan is what its backups and EDR imply in practice.
    size: "Medium (50-249)",
    readiness: "Exercised plan",
