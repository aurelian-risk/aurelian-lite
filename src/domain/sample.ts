...
export function makeSampleStudy(): Study {
  ...
  const roRansom = add("risk_origin", { name: "Ransomware group", description: "Financially motivated organized cybercrime crew.", category: "Cybercriminals", motivation: "Extortion via encryption and data theft", capability: 3, relevance: 4 });
  const roInsider = add("risk_origin", { name: "Disgruntled insider", description: "Employee with legitimate access and a grievance.", category: "Insider", motivation: "Revenge / financial gain", capability: 2, relevance: 2 });
  const roHacktivist = add("risk_origin", { name: "Hacktivist collective", description: "Ideologically motivated group seeking disruption and publicity.", category: "Hacktivist", motivation: "Protest / reputational damage", capability: 2, relevance: 2 });
  ...
  add("strategic_scenario", { name: "Insider data exfiltration", description: "Insider copies patient records to external media.", risk_origin: roInsider, stakeholder: null, feared_event: feDisclosure, likelihood: 2, gravity: 3 });
  ...
  add("security_measure", { name: "Secure email gateway & phishing training", description: "Filter malicious mail and train staff to report lures.", measure_type: "Preventive", status: "Implemented", priority: 2, covers: [st1], protects: [saHis] });
  add("security_measure", { name: "MFA on remote maintenance access", description: "Enforce phishing-resistant MFA for all third-party access.", measure_type: "Preventive", status: "Planned", priority: 3, covers: [st1, st3], protects: [saHis, saNetwork] });
  add("security_measure", { name: "Network segmentation (IT / clinical VLANs)", description: "Separate clinical VLANs from corporate IT to contain lateral movement.", measure_type: "Preventive", status: "Implemented", priority: 3, covers: [stLateral], protects: [saNetwork] });
  add("security_measure", { name: "Egress monitoring & DLP", description: "Detect and block bulk exfiltration of health records.", measure_type: "Detective", status: "Planned", priority: 3, covers: [stExfil], protects: [saHis] });
  add("security_measure", { name: "EDR on clinical endpoints", description: "Behavioural detection of credential dumping and ransomware.", measure_type: "Detective", status: "Implemented", priority: 3, covers: [st2, st3, st6], protects: [saHis] });
  add("security_measure", { name: "Offline immutable backups", description: "Air-gapped, immutable backups tested for restore.", measure_type: "Corrective", status: "Implemented", priority: 4, covers: [st6], protects: [saBackup] });
